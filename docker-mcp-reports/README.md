...
Этот проект теперь включает MCP (Model Context Protocol) сервер, который позволяет использовать все функции прямо в Cursor:
...
📖 **Подробная инструкция по MCP интеграции**: [MCP_README.md](MCP_README.md)
...
```bash
# Сборка и запуск всех сервисов (включая MCP)
./start.sh start
