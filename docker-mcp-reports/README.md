...
### 3. Python Runner MCP Server (Удаленный доступ)
...
- **Автоматическая генерация тестов** через OpenAI GPT-3.5-turbo и Claude Haiku 3.5
- **Полная интеграция** с реальной тулсой test-python-code MCP сервера

📖 **Подробные инструкции**:
- [REMOTE_ACCESS_TESTING.md](../REMOTE_ACCESS_TESTING.md) - Удаленный доступ к Python Runner
...
# Запуск Python Runner с удаленным доступом
docker-compose up -d python-runner-http tunnel-server

# Запуск Localtunnel для внешнего доступа
lt --port 8002 --subdomain nervous-grasshopper-83
